...
- init 函数, 无参无返回值, 不能被其它函数调用
- 保重的 init 函数将在 main 函数之前自动执行
- 每个包中 init 函数可以有多个, 且可以位于不同文件中
- 一个文件中至多有一个 init 函数
- 同一个包中的 init 函数没有明确的执行顺序,不可预期
- 不同包的 init 函数的执行顺序由导入顺序决定

init 函数主要是做一些初始化工作. 但是由于同一个包里面的 init 函数执行顺序不可预期, 所以, 除非有必要, 不要在同一个包里面定义多个 init. init 和 main 函数不一定在同一个文件中

`import \_ "xxx"`: 只执行该包的 init 函数,无法使用包内资源.

`import "xxx"`: 也会执行该包的 init 函数, 也可以使用包内资源.
